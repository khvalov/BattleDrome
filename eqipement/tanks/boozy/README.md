...
A tracked combat robot with an auxiliary motor, powered by MegaPi and Raspberry Pi.

---

## Core Architecture

Boozy is a two-track differential-drive platform with an auxiliary encoder motor. It uses a **MegaPi** (ATmega2560) for low-level motor and LED control, and a **Raspberry Pi Zero** as the network bridge.

- **Drive System:** 2-track differential (skid-steer — forward, reverse, in-place pivot)
- **Auxiliary motor:** Separate encoder slot controlled by D-pad RIGHT / LEFT
...
UART uses **`Serial2`** on the ATmega2560 at **115200 baud** — pre-wired to the MegaPi's RPi flex-cable connector. `Serial` (USB) is for debug output only.

---

## Firmware

**File:** `megaPi/boozy.ino`
...
| D-pad RIGHT | Aux motor forward at `AUX_SPEED` |
| D-pad LEFT | Aux motor backward at `AUX_SPEED` |
...
| SLOT2 | Aux motor | D-pad RIGHT / LEFT | `SIGN_AUX = -1` |
...
```json
{ "timestamp": 1234, "event": { "type": "rfid", "data": { "tankId": "BOOZY1", "tankType": "boozy", "uid": "A1B2C3D4" } } }
```
...
| Constant | Value | Description |
|:---|:---|:---|
| `TANK_ID` | `"BOOZY1"` | Unique identifier, up to 6 characters |
| `TANK_TYPE` | `"boozy"` | Tank model name |
...
On each shot:
1. `IrReceiver.stop()` — pauses the ISR
2. Transmits a **NEC IR frame** (address = XOR-fold of `TANK_ID`, command = `ammoLevel`)
3. `IrReceiver.start()` — resumes
4. Decrements `ammo` by 1; sends a `fire` event over `Serial2`

### Receiving hits (dual IR receivers)

Two IR receivers time-multiplexed every **200 ms**. On each valid NEC frame:
- `addr` identifies the shooter; `cmd` = damage = attacker's `ammoLevel`
- If not immune and not dead: `health -= damage`; LED blinks red ×2; sends `hit` event
- If health reaches 0: `isDead = true`, motors stop
...
```json
{
  ...
  "event": {
    "type": "telemetry",
    "data": {
      "tankId": "BOOZY1", "tankType": "boozy",
      "speed": 80, "health": 100, "ammo": 100,
      "ammoLevel": 1, "fireSpeed": 1, "immunable": false,
...
| `ammoLevel` | 1 | 1–10 | Damage per shot |
...
2. **Aux motor direction:** Flip `SIGN_AUX` if the mechanism moves the wrong way on D-pad RIGHT.
3. **IR receiver placement:** Mount one receiver on each side of the hull for 360° hit coverage.
4. **Cable management:** Use the FFC for flexible Pi positioning inside the shell.
